...
const autocut = {
  ...
  onAnalysisComplete: (callback: (data: { keepSegments: KeepSegmentData[]; srtPath: string; edlPath?: string | null; totalKeep: number; totalDuration: number; keepSpeechSec?: number; keepNonspeechSec?: number }) => void) => {
    const handler = (_: unknown, data: { keepSegments: KeepSegmentData[]; srtPath: string; totalKeep: number; totalDuration: number; keepSpeechSec?: number; keepNonspeechSec?: number }): void => callback(data)
