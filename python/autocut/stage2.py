...
def _build_context_prompt(n_frames: int) -> str:
    """5프레임 맥락 프롬프트 생성

    [이전 윈도우 끝] + [현재 시작/중간/끝] + [다음 윈도우 시작]
    """
    if n_frames == 5:
        frame_desc = (
            "프레임 1: 이전 구간 끝부분\n"
        ...
        frame_desc = (
            "프레임 1~3: 현재 구간 (시작/중간/끝)\n"
            "프레임 4: 인접 구간 참고용"
        )
    elif n_frames == 3:
        frame_desc = "프레임 1: 현재 구간 시작 / 프레임 2: 중간 / 프레임 3: 끝"
    else:
        return PROMPT_TAG

    if n_frames >= 5:
        focus = "현재 구간(프레임 2~4)에서 주인공이 무엇을 하고 있는지 판단하세요.\n앞뒤 프레임은 시간 흐름 참고용입니다."
    elif n_frames == 3:
        focus = "현재 구간의 시작/중간/끝 프레임을 종합하여 주인공이 무엇을 하고 있는지 판단하세요."
    else:
        focus = "프레임들을 종합하여 현재 구간에서 주인공이 무엇을 하고 있는지 판단하세요."

    return (
        "아웃도어 영상의 10초 구간에서 추출한 프레임들입니다.\n"
        f"{frame_desc}\n\n"
        f"{focus}"
    ) + _PROMPT_BODY
...
def tag_window_context(
    ...
    """슬라이딩 윈도우 비전 태깅 — 5프레임 맥락으로 정확도 향상

    [이전 윈도우 80%] + [현재 20%/50%/80%] + [다음 윈도우 20%]
    총 5프레임으로 현재 구간 내 행동 변화 + 앞뒤 시간 흐름을 동시에 포착합니다.
    """
    frames_b64 = []
    dur = curr_window[1] - curr_window[0]

    # 1. 이전 윈도우 끝부분 (80% 지점)
    if prev_window:
        t = prev_window[0] + (prev_window[1] - prev_window[0]) * 0.8
        frame = extract_representative_frame(video_path, t)
        if frame:
            frames_b64.append(frame)

    # 2~4. 현재 윈도우 시작/중간/끝 (20%/50%/80%)
    for ratio in (0.2, 0.5, 0.8):
        t = curr_window[0] + dur * ratio
        frame = extract_representative_frame(video_path, t)
        if frame:
            frames_b64.append(frame)
    ...
    # 5. 다음 윈도우 시작부분 (20% 지점)
    if next_window:
        t = next_window[0] + (next_window[1] - next_window[0]) * 0.2
        frame = extract_representative_frame(video_path, t)
        if frame:
            frames_b64.append(frame)
    ...
    prompt = _build_context_prompt(len(frames_b64))
    ...
    _log(f"  tag: action={label} shot={shot} usable={usable} desc={desc} ({len(frames_b64)}f)")
...
def tag_windows_batch_claude(
    ...
    try:
        # 프레임 추출 (윈도우당 시작/중간/끝 3프레임)
        frame_paths = {}
        for i, w in enumerate(windows):
            dur = w["end"] - w["start"]
            paths = []
            for fi, ratio in enumerate((0.2, 0.5, 0.8)):
                t = w["start"] + dur * ratio
                path = os.path.join(tmp_dir, f"w{i:04d}_{fi}.jpg")
                if extract_frame_to_file(video_path, t, path):
                    paths.append(path)
            if paths:
                frame_paths[i] = paths
        ...
        for batch_idx in range(total_batches):
            ...
            # 각 윈도우의 프레임 리스트를 평탄화하여 전달
            batch_paths = []
            for i in batch_indices:
                batch_paths.extend(frame_paths[i])

            # 프롬프트 구성
            window_info = "\n".join(
                f"- 윈도우 {idx} ({windows[idx]['start']:.1f}~{windows[idx]['end']:.1f}초): 프레임 {len(frame_paths[idx])}장 (시작/중간/끝)"
                for idx in batch_indices
            )

            prompt = (
                f"캠핑/아웃도어 브이로그 영상의 윈도우 {len(batch_indices)}개입니다.\n"
                f"각 윈도우는 시작/중간/끝 3프레임으로 구성되어 있습니다.\n"
                f"각 윈도우에 대해 아래 항목을 분석해주세요.\n\n"
                f"{window_info}\n\n"
                f"{_PROMPT_BODY}\n\n"
                f"각 윈도우에 대해 JSON 배열로 출력하세요:\n"
                f'[{{"window": 0, "action": "...", "shot": "...", "desc": "...", "usable": "...", "usable_reason": "..."}}]\n'
                f"window 값은 위 목록의 윈도우 번호를 그대로 사용하세요."
            )

            _log(f"Claude 배치 {batch_idx+1}/{total_batches}: 윈도우 {len(batch_indices)}개 ({len(batch_paths)}프레임)")
