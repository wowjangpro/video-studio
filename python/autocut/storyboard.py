...
def _ensure_all_files_present(
    ...
    """
    SCORE_THRESHOLD = 0  # 모든 영상 최대 활용 (규칙 2: 촬영한 영상 최대 활용)
                         # 자동 복원 segment는 _make_segment에서 5초로 trim됨
...
## ⭐ 최우선 원칙: "양보다 질" — 한 번에 끝내는 편집

**분량 맞추기보다 "꼭 필요하고 재미있는 장면과 대화만" 남기는 것이 핵심입니다.**

각 씬마다 자문:
1. 시청자에게 **가치**가 있는가? (정보/재미/감동/분위기)
2. 흐름에 **필수**인가?
3. 더 좋은 **대체 컷**이 있는가? (있으면 cut)
4. 시청자가 **지루**해할까? (정지/반복/혼잣말 = 지루)

✂️ 망설일 때는 cut. 가치 없는 장면을 keep하면 사용자가 다시 편집해야 합니다.

목표는 **재편집이 필요 없는 한 번의 좋은 결과**입니다.

## 🎯 절대 우선 규칙 3가지 (모든 결정의 기준)

### 1. 시간순 행동 흐름이 가장 중요
브이로그의 본질은 **주인공이 시간순으로 무엇을 하는가**입니다.
...
## ⭐ 최우선 원칙: 양보다 질
"꼭 필요하고 재미있는 장면과 대화만" 남기는 게 목표. 분량은 결과로 정해짐.
일반적으로 **15~25분 내외**가 적정 (캠핑 브이로그 사용자 스타일).
...
def run_narrative_editing_claude(
    ...
    try:
        ...
        if target_minutes > 0:
            ...
            default_guide = (
                f"## 분량 가이드 — 양보다 질\n\n"
                f"**분량 목표는 없습니다.** 다음 원칙에 따라 결과 분량이 자연스럽게 정해집니다:\n\n"
                f"### 핵심 원칙: \"꼭 필요하고 재미있는 장면과 대화만 keep\"\n"
                f"각 씬을 평가할 때 다음을 자문하세요:\n"
                f"1. **시청자에게 가치 있는가?** (정보 전달 / 재미 / 감동 / 분위기)\n"
                f"2. **흐름에 필수인가?** 빠지면 시간 흐름이 끊기는가?\n"
                f"3. **다른 씬으로 대체 가능한가?** 같은 활동의 다른 컷이 더 좋다면 cut\n"
                f"4. **시청자가 지루해하지 않을까?** 정지 화면/반복/혼잣말은 지루함\n\n"
                f"### 결과 분량 기준 (참고)\n"
                f"- 잘 편집된 캠핑 브이로그는 **15~25분 내외** (사용자 스타일)\n"
                f"- 원본 {total_min}분 → 완성본 {max(15, int(total_min*0.10))}~{max(25, int(total_min*0.15))}분 정도가 보통\n"
                f"- 단 영상 내용에 따라 더 짧을 수도 길 수도 있음 — 분량 강제 X\n\n"
                f"### 절대 규칙\n"
