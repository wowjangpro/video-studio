...
- **비말소리 장면이 30초 이상 이어지면 PARTIAL을 적극 고려하세요** — 같은 화면이 30초 이상 지속되면 시청자가 지루함
- 단, **흐름이 깨지지 않는 게 우선**입니다. 다음 장면과 자연스럽게 연결되는 경우에만 줄이세요
- 예시:
  - 60초 풍경 → keep_windows 3개(30초)로 줄여 다음 활동으로 자연 전환 ✅
  - 50초 셋업 과정 → 시작/완성만 남겨도 흐름이 자연스러우면 PARTIAL ✅
  - 40초 요리지만 다음 장면이 식사로 이어진다면 핵심 윈도우만 ✅
  - 그러나 흐름이 끊기거나 맥락이 사라지면 길이가 길어도 그대로 KEEP
