...
3. **정지/대기 장면** — 말소리 없고 모션 없고 아무런 변화 없이 멈춰있는 장면 (적극 제거)
4. **흔들림/NG** — 심하게 흔들리거나 의미 없는 촬영
5. **완전히 같은 내용의 반복** — 같은 행동을 같은 장소에서 반복하는 경우만
6. **저모션 정체 장면** — M:저이면서 말소리 없고 변화가 거의 없는 장면 (시청자가 지루함)
...
## 비슷한 장면이 여러 개일 때 (모션 우선)

- 같은 라벨/내용이 반복되면 **M:고 > M:중 > M:저** 순으로 남기세요
- 동일한 활동(예: 같은 자리에서의 요리, 같은 풍경)이 여러 번 나오면 모션이 가장 활발한 1~2개만 KEEP, 나머지 CUT
- 모션이 같으면 더 짧은 장면 우선 (긴 정체 장면보다 짧고 활동적인 장면)

## 길이 가이드 (말소리 제외) — 인기 캠핑 브이로그 편집 패턴 반영

**한 컷이 10초를 넘지 않게 PARTIAL로 자주 끊으세요.** 같은 화면이 길게 이어지면 시청자가 빠져나갑니다.

### 인기 캠핑 브이로그 공통 패턴
- **컷당 평균 3~7초**, 길어도 10초 이내로 끊어 줌
- **장면 전환이 빠름** — 와이드 → 클로즈업 → 디테일 → 다음 활동으로 흐름 빠르게 이동
- **같은 액티비티 안에서도 다양한 앵글**: 요리 = 재료 클로즈업 + 손동작 + 결과물 + 먹는 모습 (각 짧게)
- **B-roll 인서트**: 풍경/불꽃/장비 디테일 등을 짧게(2~5초) 사이사이 끼움
- **음악과 함께 리듬감 있게 컷이 떨어짐**

### PARTIAL 권장 패턴
- **30초 이상 비말소리 장면 → keep_windows를 띄엄띄엄** 골라 한 컷이 10초 안 넘게
  - 좋은 예: 6윈도우(60초) 장면 → `keep_windows: [0, 3]` (시작 10초 + 중간 10초, 사이 20초 점프)
  - 좋은 예: 9윈도우(90초) 셋업 → `keep_windows: [0, 4, 8]` (시작/중간/끝 각 10초)
  - 나쁜 예: `keep_windows: [0, 1, 2, 3]` (40초 연속 = 한 컷이 너무 김)
- **인접 윈도우 연속 keep은 최대 1개**까지만 — 2개 이상 연속 keep 시 한 컷이 20초 넘어 지루
- 단, **흐름이 깨지면 안 됨** — 다음 장면과 자연스럽게 연결되도록 시작/끝 윈도우는 보통 포함

### 예시
- 60초 풍경 → `keep_windows: [0, 3]` (시작 + 중후반, 다음 활동으로 전환) ✅
- 80초 셋업 → `keep_windows: [0, 3, 7]` (시작 + 중간 + 완성) ✅
- 50초 요리 (말소리 없음) → `keep_windows: [0, 4]` (재료 준비 + 완성) ✅
- 40초 식사 (말소리 없음) → `keep_windows: [0, 3]` (한입 + 마무리) ✅

## 판단 원칙

- **의심스러우면 남기세요.** 사용자가 2차 편집에서 추가로 제거할 수 있습니다.
- 이 단계는 "확실한 쓰레기 제거"이지 "좋은 편집"이 아닙니다.
- 단, 위의 "저모션 정체"와 "길이 가이드"는 적극 적용하세요 — 지루함을 줄이는 게 목적입니다.
- **장면 전환을 자주** 만들어 시청자의 몰입을 유지하세요.
